...
$app->get('/', function ($request, $response, $args){
  ...
  // Deletes unused tags to stop filling up database.
  try {
    $cleaner = new BlogPost\Tag;
    $deleteTags = BlogPost\Tag::doesntHave('posts')->get();
    foreach($deleteTags as $delTag) {
      $cleaner->where('id', $delTag['id'])->delete();
    }
  } catch (Exception $e) {
    echo 'Unable to clean up tags' . $e->getMessage;
  }
